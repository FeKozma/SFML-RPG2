...
Game::Game()
{
	...
	if (XPBlock.loadFromFile("../Resources/xp.png"))
	{
		XP.setSize(sf::Vector2f(943, 11));
		XP.setTexture(&XPBlock);
	}
	else
	{
		std::cout << "did not load XP bar texture\n";
	}
...
void Game::run()
{
	...
	sf::RenderWindow window(sf::VideoMode(gameArea.WIDTH*64, gameArea.HIGHT*64/*416, 288*/), "Subtitle...");
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			if (event.type == sf::Event::MouseButtonPressed)
			{
				...
				if (position.x > 956 && position.x < 984 && position.y > 488 && position.y < 517)
				{
					chatUp = true;
				}
...
void Game::updateWindow(sf::RenderWindow &renderWindow, float deltaTime)
{
	...
	this->isPlayerInExplosion();
	
	gameArea.update(deltaTime);
	gameArea.draw(renderWindow);

	player2->draw(renderWindow, deltaTime);
	player1->draw(renderWindow, deltaTime);
	...
	XP.setPosition(sf::Vector2f((float(64-22)), (float(64))));
	renderWindow.draw(XP);

	if (chatUp)
	{
		chatUp = chat1.update(deltaTime);
	}
	chat1.draw(renderWindow);
