...
void Blocks::setTree(int x, int y)
{
	gameArea[x][y] = 14;
	gameArea[x][y+1] = 15;
}

void Blocks::setSand(int x, int y)
{
	gameArea[x][y] = 11;
	gameArea[x][y + 1] = 11;
}
...
int Blocks::intersectRectangleCheck(const sf::Vector2f & box)
{
	...
	for (int x = 0; x < WIDTH && retValue == 0; x++)
	{
		for (int y = 0; y < HIGHT && retValue == 0; y++)   //and retValue ==0
		{
			...
			else if (typeBlock == 1 || typeBlock == 14 || typeBlock == 15 || typeBlock == 6)// && typeBlock != 2 && typeBlock != 5)
			{
		...
		for (int y = 0; y < HIGHT ; y++)
		{
			typeBlock = this->gameArea[x][y];
				if (typeBlock == 2)
				{
					if (y%3 == x % 2)
					{
						grass1.setOrigin(64 / 2, 64 / 2);
						grass1.rotate(90);
						grass1.setOrigin(0, 64);

						grass1.setPosition(sf::Vector2f((float(x * 64)), (float(y * 64))));
						renderWindow.draw(grass1);

						grass1.setOrigin(64 / 2, 64 / 2);
						grass1.rotate(360 - 90);
						grass1.setOrigin(0, 0);
					}
					else
					{
				...
				{
					if (gameArea[x][y - 1] != 6 && gameArea[x][y + 1] != 6)
					{
						thunder6.setOrigin(64 / 2, 64 / 2);
						thunder6.rotate(90);
						thunder6.setOrigin(0, 64);

						thunder6.setPosition(sf::Vector2f((float(x * 64)), (float(y * 64))));
						renderWindow.draw(thunder6);
						
						thunder6.setOrigin(64 / 2, 64 / 2);
						thunder6.rotate(360 - 90);
						thunder6.setOrigin(0, 0);
				...
				{
					if (gameArea[x][y - 1] != 7 && gameArea[x][y + 1] != 7)
					{
						thunder7.setOrigin(64 / 2, 64 / 2);
						thunder7.rotate(90);
						thunder7.setOrigin(0, 64);
						
						thunder7.setPosition(sf::Vector2f((float(x * 64)), (float(y * 64))));
						renderWindow.draw(thunder7);

						thunder7.setOrigin(64 / 2, 64 / 2);
						thunder7.rotate(360 - 90);
						thunder7.setOrigin(0, 0);
				...
				{
					if (gameArea[x][y - 1] != 8 && gameArea[x][y + 1] != 8)
					{
						thunder8.setOrigin(64 / 2, 64 / 2);
						thunder8.rotate(90);
						thunder8.setOrigin(0, 64);

						thunder8.setPosition(sf::Vector2f((float(x * 64)), (float(y * 64))));
						renderWindow.draw(thunder8);

						thunder8.setOrigin(64 / 2, 64 / 2);
						thunder8.rotate(360 - 90);
						thunder8.setOrigin(0, 0);
